...
using WinForms = System.Windows.Forms; // 用於資料夾/檔案瀏覽
...
public partial class SettingsWindow : Window
{
	...
	private void CollectFromUi()
	{
		...
		_cfg.App.MoveMode = ((System.Windows.Controls.ContentControl)CmbMoveMode.SelectedItem).Content.ToString()!;
		_cfg.App.Overwrite = ((System.Windows.Controls.ContentControl)CmbOverwrite.SelectedItem).Content.ToString()!;

		_cfg.Routing.PathTemplate = TxtPathTemplate.Text.Trim();
		_cfg.Routing.SafeCategories = ChkSafeCategories.IsChecked == true;

		_cfg.Classification.Engine = ((System.Windows.Controls.ContentControl)CmbEngine.SelectedItem).Content.ToString()!;
	...
	// 瀏覽：需要加入參考「System.Windows.Forms」
	private void BrowseRoot_Click(object sender, RoutedEventArgs e)
	{
		using var dlg = new WinForms.FolderBrowserDialog();
		if (dlg.ShowDialog() == WinForms.DialogResult.OK) TxtRootDir.Text = dlg.SelectedPath;
	}
	private void BrowseInbox_Click(object sender, RoutedEventArgs e)
	{
		using var dlg = new WinForms.FolderBrowserDialog();
		if (dlg.ShowDialog() == WinForms.DialogResult.OK) TxtInboxDir.Text = dlg.SelectedPath;
	}
	private void BrowseDb_Click(object sender, RoutedEventArgs e)
	{
		using var dlg = new WinForms.SaveFileDialog() { Filter = "SQLite (*.db)|*.db|All (*.*)|*.*" };
		if (dlg.ShowDialog() == WinForms.DialogResult.OK) TxtDbPath.Text = dlg.FileName;
	}
}
